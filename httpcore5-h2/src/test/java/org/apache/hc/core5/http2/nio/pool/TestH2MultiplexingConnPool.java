...
class TestH2MultiplexingConnPool {
    ...
    static class MockSessionState {
        ...
        void bind(final H2PoolSessionSupport support) {
            support.updatePeerSettings(peerMaxStreams.get());
    ...
    @Test
    void testStartingOneOfTwoReservedStreamsMustNotConsumeOtherReservation() throws Exception {
        final ConnectionInitiator initiator = Mockito.mock(ConnectionInitiator.class);
        final IOSession session = createMockSession();
        final MockSessionState state = new MockSessionState(2);
        setupImmediateConnect(initiator, HOST_A, session, state);

        final H2MultiplexingConnPool pool =
                new H2MultiplexingConnPool(initiator, null, null, 1, 1);

        final H2StreamLease lease1 =
                pool.lease(HOST_A, CONNECT_TIMEOUT, null).get();
        final H2StreamLease lease2 =
                pool.lease(HOST_A, CONNECT_TIMEOUT, null).get();

        // Both stream slots are reserved.
        final Future<H2StreamLease> future3 =
                pool.lease(HOST_A, CONNECT_TIMEOUT, null);

        Assertions.assertFalse(future3.isDone());

        // Stream #1 becomes active.
        //
        // This mirrors H2Streams: localCount is incremented and the
        // capacity callback is fired before the requester releases its
        // H2StreamLease reservation.
        state.activeLocalStreams.set(1);
        state.syncToSupport();
        ...
        // One active stream + one still reserved stream == full capacity.
        Assertions.assertFalse(future3.isDone());

        // Requester acknowledges that lease1 has transitioned from
        // reserved to active.
        lease1.releaseReservation();

        // lease2 is STILL reserved, therefore there must be no capacity
        // for a third stream.
        Assertions.assertFalse(
                future3.isDone(),
                "Starting lease1 must not consume lease2's reservation");
