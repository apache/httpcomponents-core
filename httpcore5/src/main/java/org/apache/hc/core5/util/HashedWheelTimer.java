...
public class HashedWheelTimer implements Timer {
     ...
     * @throws IllegalArgumentException if {@code tickDuration} is &lt;= 0
     */
    public HashedWheelTimer(final
            ThreadFactory threadFactory,final long tickDuration,final TimeUnit unit) {
    ...
    public HashedWheelTimer(
        final ThreadFactory threadFactory,
        final long tickDuration,final TimeUnit unit,final int ticksPerWheel,final boolean leakDetection) {
    ...
    private static HashedWheelBucket[] createWheel(final int ticksPerWheel) {
        ...
        final int normaledTicksPerWheel = normalizeTicksPerWheel(ticksPerWheel);
        final HashedWheelBucket[] wheel = new HashedWheelBucket[normaledTicksPerWheel];
    ...
    @Override
    public WheelTimeout newTimeout(final TimerTask task,final long delay,final TimeUnit unit) {
        if (task == null) {
        ...
        final long pendingTimeoutsCount = pendingTimeouts.incrementAndGet();
        ...
        final long deadline = System.nanoTime() + unit.toNanos(delay) - startTime;
    ...
    private final class Worker implements Runnable {
        private final Set<WheelTimeout> unprocessedTimeouts = new HashSet<WheelTimeout>();
        ...
        public void run() {
            ...
            do {
                ...
                if (deadline > 0) {
                    final int idx = (int) (tick & mask);
                    processCancelledTasks();
                    final HashedWheelBucket bucket =
                            wheel[idx];
            ...
            for (final HashedWheelBucket bucket: wheel) {
                bucket.clearTimeouts(unprocessedTimeouts);
            }
            for (;;) {
                final HashedWheelTimeout timeout = timeouts.poll();
                if (timeout == null) {
                    break;
                }
        ...
        private void transferTimeoutsToBuckets() {
            ...
            for (int i = 0; i < 100000; i++) {
                ...
                final long calculated = timeout.deadline / tickDuration;
                timeout.remainingRounds = (calculated - tick) / wheel.length;

                final long ticks = Math.max(calculated, tick); // Ensure we don't schedule for past.
                final int stopIndex = (int) (ticks & mask);

                final HashedWheelBucket bucket = wheel[stopIndex];
        ...
        private void processCancelledTasks() {
            for (;;) {
                final HashedWheelTimeout timeout = cancelledTimeouts.poll();
                if (timeout == null) {
                    // all processed
                    break;
                }
        ...
        private  String systemProperty(final String key,final String def) {
            if (key == null) {
        ...
        private boolean isWindows() {
            final boolean windows = systemProperty("os.name", "").toLowerCase(Locale.US).contains("win");
            if (windows) {
        ...
        private long waitForNextTick() {
            final long deadline = tickDuration * (tick + 1);
    ...
    private static final class HashedWheelTimeout implements WheelTimeout {
        ...
        public String toString() {
            ...
            final long remaining = deadline - currentTime + timer.startTime;

            final StringBuilder buf = new StringBuilder(192)
               .append("HashedWheelTimeout")
               .append('(')
               .append("deadline: ");
            if (remaining > 0) {
    ...
    private static final class HashedWheelBucket {
         ...
         */
        public void clearTimeouts(final Set<WheelTimeout> set) {
            for (;;) {
                final HashedWheelTimeout timeout = pollTimeout();
                if (timeout == null) {
                    return;
                }
